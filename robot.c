...
// MAZE 1
  ...
  /*  void setup_robot(struct Robot *robot){
    robot->x = OVERALL_WINDOW_WIDTH/2-30;
    robot->y = OVERALL_WINDOW_HEIGHT/2-10;
    robot->true_x = OVERALL_WINDOW_WIDTH/2-30;
...
int checkRobotSensorRightFrontAllWalls(struct Robot * robot, struct Wall_collection * head) {
    struct Wall_collection *ptr, *head_store;
    int i;
    double xDir, yDir;
    int robotCentreX, robotCentreY, xBR, yBR;
    int score, hit;
    ...
    for (i = 0; i < 5; i++)
    {
        ...
        while(ptr != NULL) {
            hit = (hit || checkRobotSensor(xBR, yBR, sensorSensitivityLength, &ptr->wall));
            ptr = ptr->next;
        }
        if (hit)
            score = i;
...
int checkRobotSensorRightBackAllWalls(struct Robot * robot, struct Wall_collection * head) {
    ...
    {
        ptr = head_store;
        xDir = round(robotCentreX-(-ROBOT_WIDTH/2-SENSOR_VISION+sensorSensitivityLength*i)*cos((robot->angle)*PI/180)-(ROBOT_HEIGHT/2-2)*sin((robot->angle)*PI/180));
        yDir = round(robotCentreY-(-ROBOT_WIDTH/2-SENSOR_VISION+sensorSensitivityLength*i)*sin((robot->angle)*PI/180)+(ROBOT_HEIGHT/2-2)*cos((robot->angle)*PI/180));
        xBR = (int) xDir;
        yBR = (int) yDir;
        hit = 0;
...
void robotUpdate(struct SDL_Renderer * renderer, struct Robot * robot){
    ...
    */
    ...
    SDL_RenderDrawLine(renderer,xTR, yTR, xBR, yBR); //right
    SDL_RenderDrawLine(renderer,xBR, yBR, xBL, yBL); //back
    SDL_RenderDrawLine(renderer,xBL, yBL, xTL, yTL); //left
    SDL_RenderDrawLine(renderer,xTL, yTL, xTR, yTR); //front
    ...
    for (i = 0; i < 5; i++)
    {
        ...
        SDL_Rect rect = {xBR, yBR, 2, sensor_sensitivity};
        SDL_SetRenderDrawColor(renderer, 80+(20*(5-i)), 80+(20*(5-i)), 80+(20*(5-i)), 255);
        SDL_RenderDrawRect(renderer, &rect);
        SDL_RenderFillRect(renderer, &rect);
    }
    ...
    for (i = 0; i < 5; i++)
    {
        xDir = round(robotCentreX-(-ROBOT_WIDTH/2-SENSOR_VISION+sensor_sensitivity*i)*cos((robot->angle)*PI/180)-(ROBOT_HEIGHT/2-2)*sin((robot->angle)*PI/180));
        yDir = round(robotCentreY-(-ROBOT_WIDTH/2-SENSOR_VISION+sensor_sensitivity*i)*sin((robot->angle)*PI/180)+(ROBOT_HEIGHT/2-2)*cos((robot->angle)*PI/180));
        xBR = (int) xDir;
        yBR = (int) yDir;
...
void robotAutoMotorMove(struct Robot * robot, int front_left_sensor, int front_right_sensor, int right_front_sensor, int right_back_sensor) {
    ...
    else if ((front_left_sensor == 0) && (front_right_sensor == 0) && (right_front_sensor >= 2) && (right_back_sensor >= 2)) {
        if (robot->currentSpeed<5)
            robot->direction = UP;
    }
    ...
    else if ((robot->currentSpeed>0) && ((front_left_sensor >= 2) && (front_right_sensor == 0)) ) {
        robot->direction = DOWN;
    }
    else if ((robot->currentSpeed==0) && ((front_left_sensor >= 2) && (front_right_sensor == 0)) ) {
        robot->direction = LEFT;
